...
- [x] optionsページを開くと、現在保存されている設定(初回は初期値)がフォームの
      各項目(有効/無効・角度・ディレイフレーム数・自動再開)に反映されている
      — 確認日: 2026-08-01(Surface実機)。問題無し
- [x] 「拡張機能を有効にする」をOFFにすると、YouTube/Prime Videoを開いていても
      カメラが即座に止まる(F-20)。ONに戻すと再びカメラが起動する
      — 確認日: 2026-08-01(Surface実機)。挙動自体は問題無いが、
      offscreen documentのコンソールに`カメラ停止`ログが出力されないことを確認。
      原因未特定。実害が無いためユーザー判断で調査を見送ることに決定(2026-08-01)
- [x] 判定角度のしきい値を変更すると、変更後の値がoffscreenの検出ループに
      即座に反映される(体感で確認: 角度を小さくすると少し首を振っただけで
      反応するようになる)(F-21)
      — 初回確認(2026-08-01、Surface実機)で**反映されないバグを発見**
      (20→5に変更しても体感が全く変わらない)。原因はoffscreen document内で
      `chrome.storage`が(拡張機能の完全な削除・再読み込み後も)`undefined`になる
      事象(Phase7の別バグ修正で見つけたものと同一)で、設定変更が
      offscreen側に一切届いていなかった。offscreen documentがchrome.storageを
      直接読みに行くのをやめ、backgroundからメッセージ(START_CAMERA /
      SETTINGS_UPDATED)で設定を渡す設計に変更して対応。
      再検証日: 2026-08-01(Surface実機)。問題無し
- [x] ディレイフレーム数を変更すると、一時停止までの反応速度が体感で変わる
      (数値を小さくすると素早く反応するようになる)(F-21)
      — 初回確認(2026-08-01、Surface実機)で**反映されないバグを発見**
      (1→20に変更しても体感が全く変わらない)。上記と同一原因、同一修正。
      再検証日: 2026-08-01(Surface実機)。問題無し
- [x] 自動再開のチェックを外すと、視聴に戻っても自動再生されず、手動で
      再生ボタンを押すまで一時停止のままになる(F-11/F-21)
      — 確認日: 2026-08-01(Surface実機)。問題無し
      (この項目はservice-worker.ts側の判定のため、offscreen document側の
      chrome.storage不具合の影響を受けなかったと見られる)
- [x] 角度・ディレイフレーム数の入力欄にmin/max範囲外の値やカラ欄を入れて
      フォーカスを外しても、拡張機能がエラーで壊れず、妥当な範囲の値に
      補正されて保存される(clampSettingNumber()の動作確認)
      — 確認日: 2026-08-01(Surface実機)。問題無し
- [x] 設定を変更するたびに「保存しました」のメッセージが一時的に表示される
      — 確認日: 2026-08-01(Surface実機)。問題無し
- [x] optionsページを閉じて開き直しても、直前に変更した設定が保持されている
      — 確認日: 2026-08-01(Surface実機)。問題無し

## 手動一時停止が視線復帰で勝手に再生されないこと(F-11のバグ修正)

要手動確認。Surface実機での実際の操作でのみ発生を確認できたバグへの対応
(2026-08-01、Phase7のF-20/F-21実装後に発見)。`pausedByExtension`をタブ単位で
`chrome.storage.session`に保存する設計に変更したため、以下を確認する。

- [x] YouTube動画を再生 → 手動で一時停止 → よそ見 → 視線を戻す、という操作をしても
      動画が再生されないまま(手動で一時停止した動画を拡張機能が勝手に再生しない)
      — 確認日: 2026-08-01(Surface実機)。問題無し
- [x] YouTube動画を再生したまま(一時停止せず) → よそ見(拡張機能が自動でpause)
      → 視線を戻す、という操作では従来通り自動で再生が再開する(自動再開自体が
      壊れていないことの確認)
      — 確認日: 2026-08-01(Surface実機)。問題無し
- [x] 拡張機能が自動でpauseした後、30秒以上(service workerがアイドルで終了する目安)
      視線を外したまま待ってから視線を戻しても、正常に自動再開する(service worker
      再起動をまたいでpausedByExtensionの記録が保持されていることの確認。
      `chrome://extensions`のservice workerリンクが「非アクティブ」表示になってから
      視線を戻すとよい)
      — 確認日: 2026-08-01(Surface実機)。問題無し
- [x] タブA(YouTube、拡張機能が自動pause中)からタブB(別のYouTube動画、ユーザーが
      手動で一時停止済み)にアクティブタブを切り替えてから視線を戻しても、
      タブBの動画が勝手に再生されない(タブごとに状態が分離されていることの確認)
      — 確認日: 2026-08-01(Surface実機)。問題無し

**既知の制約**(code-reviewerレビューで指摘、意図した安全側の設計):
複数タブを同時に視聴している場合、拡張機能が自動一時停止させたタブから
一度離れて別タブに切り替えると、そのタブへ戻ってから改めて「よそ見→視線を戻す」の
遷移が起きるまで自動再開されない(離れている間はCONFIRMED_STATE_CHANGEDが
