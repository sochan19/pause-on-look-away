...
- [ ] YouTube動画を再生 → 手動で一時停止 → よそ見 → 視線を戻す、という操作をしても
      動画が再生されないまま(手動で一時停止した動画を拡張機能が勝手に再生しない)
- [ ] YouTube動画を再生したまま(一時停止せず) → よそ見(拡張機能が自動でpause)
      → 視線を戻す、という操作では従来通り自動で再生が再開する(自動再開自体が
      壊れていないことの確認)
- [ ] 拡張機能が自動でpauseした後、30秒以上(service workerがアイドルで終了する目安)
      ...
      視線を戻すとよい)
- [ ] タブA(YouTube、拡張機能が自動pause中)からタブB(別のYouTube動画、ユーザーが
